...
scatter(data.chi, y, s=10, label = L"\mathrm{Data}", c = "k", zorder = 0)
# plot data where Wf > 0.12
xlims, ylims = ax.get_xlim(), ax.get_ylim()

plot(ci.x, model(ci.x, popt), color = "C1", label = L"\mathrm{Fit}")
fill_between(ci.x, ci.c0, ci.c1, color = "C1", alpha = 0.2, label = L"2\sigma\ \mathrm{confidence\ band}")

text(0.53, 0.85, L"f(\chi) = - \frac{5040}{T}\chi + \log\left( \frac{N_{\mathrm{Fe}}}{N_{\mathrm{H}}} \right)", fontsize=14, transform=ax.transAxes)
text(0.53, 0.71, L"T = 6100(300)\ K", fontsize=14, transform=ax.transAxes)
text(0.53, 0.59, L"\frac{N_{\mathrm{Fe}}}{N_{\mathrm{H}}} = 0.011(4)\ \%", fontsize=14, transform=ax.transAxes)
rect = mpl.patches.FancyBboxPatch((0.54, 0.55), 0.4, 0.39, linewidth=1.5, edgecolor="C1", facecolor="none", transform=ax.transAxes, boxstyle=mpl.patches.BoxStyle("Round", pad=0.02))
ax.add_patch(rect)
...
fill_betweenx([-10, 0], -3.3, color = "crimson", alpha = 0.2)

text(0.36, .12, L"\mathrm{Linear\ regime}", ha = "right", color = "k", fontsize=14, transform=ax.transAxes)
text(0.36, .05, L"\propto N", color = "k", ha = "right", fontsize=14, transform=ax.transAxes)

text(0.43, .12, L"\mathrm{Saturation\ regime}", color = "crimson", fontsize=14, transform=ax.transAxes)
text(0.43, .05, L"\propto \sqrt{\ln N}", color = "crimson", fontsize=14, transform=ax.transAxes)
scatter(nom.(x2[x2 .> -3.3]), y2[x2 .> -3.3], label = L"\mathrm{Excluded\ data}", color = "crimson", s = 10)
...
text(0.53, 0.85, L"f(\chi) = - \frac{5040}{T}\chi + \log\left( \frac{N_{\mathrm{Fe}}}{N_{\mathrm{H}}} \right)", fontsize=14, transform=ax.transAxes)
text(0.53, 0.71, L"T = 5530(110)\ K", fontsize=14, transform=ax.transAxes)
text(0.53, 0.59, L"\frac{N_{\mathrm{Fe}}}{N_{\mathrm{H}}} = 0.043(7)\ \%", fontsize=14, transform=ax.transAxes)
rect = mpl.patches.FancyBboxPatch((0.54, 0.55), 0.4, 0.39, linewidth=1.5, edgecolor="C1", facecolor="none", transform=ax.transAxes, boxstyle=mpl.patches.BoxStyle("Round", pad=0.02))
ax.add_patch(rect)
